...
export function loadTranscriptionsIndex() {
  ...
  if (data && Array.isArray(data.items)) {
    ...
    const items = data.items.filter((it) => {
      if (it.source_only) return false;
      const path = String(it.path || it.filepath || '');
      if (path.startsWith('content/transcriptions/')) return false;
      const st = it.pipelineStatus;
      const visible =
        st === 'ai_processed' || st === 'ready_for_site' || it.readyForSite === true;
      return visible;
    });
...
function findIndexItemById(index, id) {
  ...
  for (const candidate of index.items) {
    if ((candidate.childIds || []).includes(id)) return candidate;
    for (const children of Object.values(candidate.sourceEntries || {})) {
      if ((children || []).some((child) => child && child.id === id)) return candidate;
    }
  }
  return null;
}
