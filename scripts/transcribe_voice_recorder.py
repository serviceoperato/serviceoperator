...
def run_phase2_for_raw(raw_path: Path, registry: dict) -> None:
    """Phase 2 is Chat Composer only — mark pending; user runs AI in Cursor chat."""
    log.info(
        "Phase 2 deferred to Cursor chat for %s (see content/voice-reports/cursor-voice-ai-prompt.md)",
        raw_path.name,
    )
    write_progress(message=f"Pending AI (chat): {raw_path.name}")
